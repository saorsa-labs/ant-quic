...
# Validates the current pure-PQC release surface.
#

set -e

echo "=== PQC Release Validation ==="
echo "Date: $(date)"
...
echo "Testing stripped feature surface..."
cargo check --no-default-features --quiet 2>/dev/null
print_status $? "No-default-features build compiles"

echo "Testing default feature surface..."
cargo check --quiet 2>/dev/null
print_status $? "Default feature set compiles"

echo "Testing all declared features..."
...
echo "Running clippy across all declared features..."
cargo clippy --all-features --all-targets -- -D warnings >/dev/null 2>&1
print_status $? "Clippy passes with all features"

# 4. Test Suite
print_header "Test Suite"

echo "Running basic PQC integration tests..."
cargo test --test pqc_basic_integration --quiet
print_status $? "Basic PQC integration tests pass"

echo "Running PQC config tests..."
cargo test --test pqc_config --quiet 2>/dev/null
print_status $? "PQC config tests pass"

# 5. Documentation Build
print_header "Documentation"

echo "Building documentation..."
cargo doc --all-features --no-deps --quiet 2>/dev/null
...
if cargo test --test pqc_basic_integration test_pqc_config_builder --release 2>/dev/null; then
...
echo -e "${GREEN}✓ Release script is using the current Cargo.toml version${NC}"

if [ -f "CHANGELOG.md" ]; then
    if grep -q "$CARGO_VERSION" CHANGELOG.md; then
        echo -e "${GREEN}✓ CHANGELOG.md contains v$CARGO_VERSION entry${NC}"
    else
        echo -e "${YELLOW}⚠ CHANGELOG.md needs a v$CARGO_VERSION entry${NC}"
    fi
else
...
echo "  ✓ Feature surface: Minimal and current"
echo "  ✓ PQC algorithms: ML-KEM-768, ML-DSA-65"
echo "  ✓ Pure PQC mode: Enforced"
...
if [ "$VALIDATION_PASSED" = true ]; then
    ...
    echo "2. Create git tag: git tag -a v${CARGO_VERSION} -m 'Release v${CARGO_VERSION}'"
    echo "3. Push to GitHub: git push origin v${CARGO_VERSION}"
