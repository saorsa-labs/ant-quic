...
fn write_frame_type(frame_type: u64) -> Result<BytesMut, Box<dyn Error>> {
...
fn payload_after_frame_type(
    ...
    let expected_prefix = write_frame_type(expected_frame_type)?;
...
fn append_addr(buf: &mut BytesMut, address: SocketAddr) {
    match address {
        SocketAddr::V4(addr) => {
...
fn expected_add_address(
    ...
    let mut expected = write_frame_type(frame_type)?;
    expected.write_var(sequence.into_inner())?;
    append_addr(&mut expected, address);
    Ok(expected)
}
...
fn remove_address_uses_production_codec() -> Result<(), Box<dyn Error>> {
    ...
    let mut expected = write_frame_type(REMOVE_ADDRESS)?;
    expected.write_var(sequence.into_inner())?;
