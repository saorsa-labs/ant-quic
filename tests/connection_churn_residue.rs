...
/// #368 variant (e) — the orphan-connection regression harness: B streams
/// continuously, then B's endpoint is dropped WITHOUT close (kill), a new B
/// with the SAME identity reconnects while A's old connection lingers.
/// 30 cycles. On A, `buffered_bytes_totals().recv_streams_with_unread`
/// must return to baseline within 15 s of each cycle (no ratchet), and
/// every lifecycle entry without a reader must be closed within 15 s
/// (`orphan_connections_closed` counts any race-produced orphan).
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore]
async fn churn_residue_orphan_connection_no_ratchet() {
    ...
    let baseline_streams = a.buffered_bytes_totals().2;
    println!("[e-orphan cycle  0] baseline recv_streams_with_unread={baseline_streams}");

    let mut max_linger = 0usize;
    for cycle in 1..=30 {
        // B connects and streams.
        let b = make_node(vec![a_addr]).await;
        let _accept_b = spawn_accept_loop(b.clone());
        let a_id = a.peer_id();
        ...
        // Kill WITHOUT close: drop the endpoint entirely (no disconnect).
        drop(b);

        // A new B (fresh endpoint, same role) reconnects while A's old
        // connection lingers in lifecycle.
        let b2 = make_node(vec![a_addr]).await;
        let _accept_b2 = spawn_accept_loop(b2.clone());
        ...
        // Wait up to 15 s for A's unread-stream count to return to baseline.
        let settled = timeout(Duration::from_secs(15), async {
            loop {
                if a.buffered_bytes_totals().2 <= baseline_streams {
                    return true;
                }
        ...
        max_linger = max_linger.max(streams_now);
        if cycle % 10 == 0 {
            println!(
                "[e-orphan cycle {cycle:2}] streams_now={streams_now} baseline={baseline_streams} orphan_closes={} open={}",
                a.orphan_connections_closed(),
                a.quic_open_connections(),
            );
        }
        // No ratchet: allow brief settling but never a persistent climb.
        assert!(
            streams_now <= baseline_streams.saturating_add(4),
            "cycle {cycle}: recv_streams_with_unread ratcheted to {streams_now} (baseline {baseline_streams})"
        );
        let _ = settled;
    }
    println!(
        "[e-orphan done] orphan_closes={} final_open={} max_linger={max_linger}",
        a.orphan_connections_closed(),
        a.quic_open_connections(),
    );
}
