...
mod tests {
    ...
    async fn create_test_cache(temp_dir: &TempDir) -> Arc<BootstrapCache> {
        ...
        Arc::new(BootstrapCache::open(config).await.expect("Failed to create cache"))
    }
    ...
    async fn multiple_tokens_different_peers() {
        ...
        assert_eq!(
            store.take(&peer1),
            Some(Bytes::from_static(b"token1"))
        );
        assert_eq!(
            store.take(&peer2),
            Some(Bytes::from_static(b"token2"))
        );
        assert_eq!(
            store.take(&peer3),
            Some(Bytes::from_static(b"token3"))
        );
